...
description: "BasicTextField 가 가진 자체 undo 와 우리 멀티 탭 모델이 서로 다른 시간선 위에서 살던 문제, 그리고 한 글자씩 들어간 편집이 한 번의 Ctrl+Z 로 통째로 휘말리던 보폭 문제"
...
우회는 **이벤트를 더 일찍 가로채는 것**이다. Compose 의 `Window.onPreviewKeyEvent` 는 포커스된 위젯이 이벤트를 보기 *전에* 한 번 발화한다. 거기서 Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y 를 잡아 우리 히스토리로 처리하고 `true` 를 돌려 이벤트를 소비하면, `BasicTextField` 는 그 키 입력을 보지 못한다.
