...
고등학교 때 Flutter + Firebase로 학교 앱을 1인 개발했다. Java로 프로토타입을 만들다가 Flutter로 넘어왔고, 급식, 시간표, 게시판, 채팅, 성적 관리까지 들어간 앱을 혼자 만들었다.

코딩은 독학이다. 학교에서 따로 가르쳐주는 건 없었고, 기숙사 자습 시간과 주말에 만들었다. 시험 기간에는 손을 놓고, 끝나면 다시 잡고.
...
앱을 만들면서 겪은 것들을 기록하는 곳이다. 기술 선택의 이유, 삽질한 과정, 구조를 바꾼 이유 같은 것들. 튜토리얼이 아니라 개발 일지에 가깝다.
